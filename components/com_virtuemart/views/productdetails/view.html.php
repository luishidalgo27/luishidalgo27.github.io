 ...
 * @version $Id: view.html.php 10745 2022-11-21 09:56:13Z Milbo $
 */
    ...
    /**
		...
		function display($tpl = null) {
			...
			if ($category_model) {
				...
				$this->category = $category_model->getCategory($product->virtuemart_category_id, $this->cat_productdetails);
				if($this->category->parents===null) $this->category->parents = $category_model->getParentsList($product->virtuemart_category_id);
				if(in_array($last_category_id,$product->categories) && !$seo_full) $product->category_name = $this->category->category_name;

				$category_model->addImages($this->category, 1);
				if($this->cat_productdetails){
					$category_model->addImages($this->category->children, 1);
				}


				if ($this->category->parents and !empty($menu->query['view']) and $menu->query['view']!='productdetails') {
			...
			if ($product->customtitle) {
				...
				$catName = '';
				if($this->category->published and !empty($this->category->category_name)){
			...
			if ($product->metadesc) {
				...
				$document->setDescription( strip_tags(html_entity_decode($product->product_name,ENT_QUOTES)) . " " . $this->category->category_name . " " . strip_tags(html_entity_decode($product->product_s_desc,ENT_QUOTES)) );
			}
