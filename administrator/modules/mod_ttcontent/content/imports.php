<?php
				...
				// if files exists in content upload it to the upload directory
				$files_folder_exists = JFolder::exists($files_path); 
				...
				function create_category()
				{
				 ...
				 elseif ($con_info['item_format'] == "Numeric") { $field_type = 'number';
				 $min_length = '10';
				 $max_length = '10'; }
				 else $field_type = 'url';
				 if($con_info['ttr_item_idreq'] == "off") { $required='0';
				 $custom_error = ''; }
				 else { $required='1';
				 $custom_error = 'Please fill the required fields.'; }	 
				 $fields +=array('fields'.$fieldcounter => array('name' => str_replace(' ','-',strtolower($field_label)),'type' => $field_type,'label' => $field_label,'required' => $required,'custom_error' => $custom_error,'show_label' => '1','options' => '','width' => '12','placeholder' => '','textarearows' => '4','min_length' => $min_length,'max_length' => $max_length,'min' => '','max' => '','optionslayout' => 'inline','calendar_min' => '','calendar_max' => '','calendar_format' => '',),);
				 $fieldcounter += 1;
				 ...
				 $fields +=array('fields'.$fieldcounter => array('name' => str_replace(' ','-',strtolower($message_label)),'type' => 'textarea','label' => $message_label,'required' => '0','custom_error' => '','show_label' => '1','options' => '','width' => '12','placeholder' => '','textarearows' => '4','min_length' => '','max_length' => '','min' => '','max' => '','optionslayout' => 'inline','calendar_min' => '','calendar_max' => '','calendar_format' => '',),);
				 $fieldcounter += 1;
				 ...
				 $fields +=array('fields'.$fieldcounter => array('name' => 'file','type' => 'file','label' => 'File','required' => '0','custom_error' => '','show_label' => '1','options' => '','width' => '12','placeholder' => '','textarearows' => '4','min_length' => '','max_length' => '','min' => '','max' => '','optionslayout' => 'inline','calendar_min' => '','calendar_max' => '','calendar_format' => '',),);
				 ...
				 'core.delete' => array(0),
				 'core.edit' => array(0),
				 'core.edit.state' => array(0),
				 'module.edit.frontend' => array(0),
				 ),
				 'params' => array(
				 'title' => '',
				 'description' => '',
				 ...
				 $cform = JModelLegacy::getInstance('Module', 'ModulesModel', array('ignore_request' => true));
				...
				private function parse_items($item_name ,$item, &$contactus_info)
					...
					if(isset($item->attributes()->ContentStringFormat) && !empty($item->attributes()->ContentStringFormat))
					$contentstring = (string)$item->attributes()->ContentStringFormat;
					else
					$contentstring = "NoValidation";
				...
				'rights' =>"",
				'xreference'=>""),
				'com_fields'=>array('ttpageid'=> $page['meta_id'])
				);
				...
				$db->setQuery($query);
				$db->execute();
				/*$db->setQuery("SELECT menu.id FROM #__menu_types menu WHERE menu.title='theme-200123-j3-menu' AND menu.menutype='custommenu'");
				$mid = $db->loadResult();
				if(empty($mid))
				{*/
				// create TT-menu
			...
			private function upload_media($type)
				{
					jimport('joomla.filesystem.folder');
					$path = JPATH_SITE . "/"."images"."/theme-200123-j3/";
					$mode = 0755;
				...
				while ($tt_read_image = readdir($tt_content_images)) 
				{
					...
					{
					$var2 = implode(DS,$rootFolder). "/"."images"."/theme-200123-j3/" . $tt_read_image;
				...
				{
				$url1 = JURI::root().'images/theme-200123-j3';
				...
				$post = preg_replace_callback('/(href=)([\'"])([\/\\\]?files[\/\\\]?)(.*?)\2()/', array($this,'tt_media_src'), $post);
