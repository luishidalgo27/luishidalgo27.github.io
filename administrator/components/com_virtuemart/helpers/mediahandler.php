...
class VmMediaHandler {
	...
	function displayMediaThumb($imageArgs=array(),$lightbox=true,$effect="rel='group'",$return = true,$withDescr = false,$absUrl = false, $width=0,$height=0){
		...
		if(empty($this->file_name) or empty($this->file_url)){
			vmdebug('displayMediaThumb empty file name or url '.$this->virtuemart_media_id,$this->file_name,$this->file_url);
...
<td>';
		...
		if(!empty($imgWidth)){
			...
			$imgWidth = 'max-width:500px;width:auto;';
		}
		...
		$minWidth = intval(VmConfig::get('img_width',90))+10;
		$html .= '<td rowspan = "8" style="overflow: hidden;" >';
