...
/**
	...
	static function ensureMemoryLimit($minMemory=0){
		...
		if($minMemory === 0) $minMemory = VmConfig::getInt('minMemory','128');
	...
	static function getMemoryLimitBytes(){
		...
		if($mLimit===null){
			...
			if($mLimit<=0) $mLimit = ((int)VmConfig::get('givenMaxRam', 2048)) * 1024 * 1024;
	...
	static function getMemoryLimit(){
		...
		if($mLimit===null){
			...
			if($mLimit<=0) $mLimit = (int)VmConfig::get('givenMaxRam', 2048);
	...
	static function getExecutionTime(){
		...
		if(empty($max_execution_time) || ($max_execution_time <= 0)){
			$max_execution_time = (int) VmConfig::get('givenMaxTime',120);
...
//In WP, we run the define, when we render vm, in Joomla we have to run them here
		...
		if(!self::$installed){
			...
			if(!empty($msg)){
				...
				$app->enqueueMessage(vmText::_($msg), vmEcho::$mType);
			}
	...
	static function getFloat($key, $default=0.0){
	...
	static public function updateDbEntry(){
		$user = JFactory::getUser();
		if($user->authorise('core.admin','com_virtuemart')){
			...
			$confData['config'] = self::$_jpConfig->toString();
