 ...
 * @version $Id: product.php 10771 2022-12-19 19:24:30Z Milbo $
 */
...
class VirtueMartModelProduct extends VmModel {
	...
	protected function populateState () {
		...
		if (VmConfig::isSite()) {
			...
			//vmdebug('my keyword for search',$this->keyword);
...
WHERE (' . implode (" AND \n ", $queryArray[2]) . ') AND p.`virtuemart_product_id`!="'.$product->virtuemart_product_id.'" '."\n";
			...
			foreach ($neighbors as &$neighbor) {
				...
				$k = 0;
				if(isset($orderByNames['prop'][$k])){
					$name = $orderByNames['prop'][$k];
					...
					$qm .= $orderByNames['select'][$k].' '.$op.' "'.$pName.'" AND ';
					//vmdebug('getneighbors '.$qm,$op);
				}
				$qm = substr($qm, 0, -4);
				$qm .= '
